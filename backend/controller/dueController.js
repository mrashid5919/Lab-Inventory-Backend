...
const createDue = async (req, res) => {
  ...
  const dueExist = await pool.query("SELECT * from dues where req_id=$1", [
    reqID,
  ]);
  ...
  const requ = await pool.query("SELECT * from requests where req_id=$1", [
    reqID,
  ]);
  ...
  try {
    if (dueExist.rows.length == 0) {
      ...
      const due = await pool.query(
        "INSERT INTO dues(req_id,due_date,alloter_id,due_status,issue_date,quantity,damage_quantity,monetary_assigned) values($1,$2,$3,$4,CURRENT_DATE,$5,$6,$7) RETURNING *",
        [
          reqID,
          dueDateObj,
          user_id.rows[0].user_id,
          duestatus.rows[0].due_status,
          requ.rows[0].quantity,
          0,
          0,
        ]
      );
      ...
      const due = await pool.query(
        "UPDATE dues SET due_date=$1 where req_id=$2 RETURNING *",
        [dueDateObj, reqID]
      );
      //const d=await pool.query("SELECT due_id from dues where req_id=$1",[reqID]);
      //console.log(due.rows[0]);
...
const viewDuesStudent = async (req, res) => {
  ...
  try {
    ...
    const dues = await pool.query(
      `SELECT d.due_id,d.due_date,d.issue_date,ds.status_name,e.equipment_name,l.location_name,d.quantity,d.clear_date,d.damage_quantity from dues d
...
const viewDuesLocation = async (req, res) => {
  ...
  try {
    const dues = await pool.query(
      `SELECT d.due_id,d.due_date,d.issue_date,ds.status_name,e.equipment_name,u1.username,d.quantity,d.clear_date,d.damage_quantity from dues d
        join requests r on d.req_id=r.req_id
        join equipments e on r.equipment_id=e.equipment_id
  ...
  try {
    ...
    const notification = await pool.query(
      ...
      [
        ...
        username,
        "Lab Assistant",
        "A due has been cleared",
        notif_type.rows[0].notification_type,
        dueID,
      ]
    );
    const clear = await pool.query(
      "UPDATE dues SET due_status=$1,clear_date=CURRENT_DATE,receiver_id=$2 where due_id=$3 RETURNING *",
      [duestatus.rows[0].due_status, user_id.rows[0].user_id, dueID]
    );
...
const viewLostorDamaged = async (req, res) => {
  ...
  try {
    const dues = await pool.query(
      `SELECT d.due_id,d.due_date,d.issue_date,ds.status_name,e.equipment_name,u1.username,d.quantity,d.clear_date,d.damage_quantity,d.monetary_assigned from dues d
        join requests r on d.req_id=r.req_id
        join equipments e on r.equipment_id=e.equipment_id
        join users_in_locations ul on r.location_id=ul.location_id
        join due_statuses ds on d.due_status=ds.due_status
        join users u1 on r.user_id=u1.user_id
        join users u on ul.user_id=u.user_id
        where u.username=$1 and ds.status_name='LostOrDamaged'`,
      [username]
    );
    res.status(200).json(dues.rows);
  } catch (err) {
...
const showEstimateDue = async (req, res) => {
  ...
  try {
    ...
    const equipment_cost = await pool.query(
      `SELECT e.cost*d.damage_quantity AS estimated_cost FROM equipments e
...
const createMonetaryDue = async (req, res) => {
  ...
  try {
    const due = await pool.query("SELECT * from dues where due_id=$1", [dueID]);
    const dueExist = await pool.query(
      "SELECT * from monetary_dues where req_id=$1",
      [due.rows[0].req_id]
    );
    ...
    if (dueExist.rows.length == 0) {
      ...
      const dueupdate = await pool.query(
        "UPDATE dues SET monetary_assigned=$1 WHERE due_id=$2",
        [1, dueID]
      );
      const monetary_due = await pool.query(
        "INSERT INTO monetary_dues(req_id,user_id,creater_id,amount,due_status,issue_date,due_date,damage_quantity) values($1,$2,$3,$4,$5,now(),$6,$7) RETURNING *",
        [
          due.rows[0].req_id,
          requ.rows[0].user_id,
          due.rows[0].alloter_id,
          ...
          duestatus.rows[0].due_status,
          dueDateObj,
          due.rows[0].damage_quantity,
        ]
      );
      const notif_status = await pool.query(
        "SELECT * from notification_types where type_name='MonetaryDues'"
      );
      const notification = await pool.query(
        "INSERT INTO notifications(receiver_id,sender_name,sender_role,notification,notification_time,notification_type,type_id) VALUES ($1,$2,$3,$4,now(),$5,$6) RETURNING *",
        [
          ...
          username,
          "Lab Assistant",
          "A monetary due has been created",
          notif_status.rows[0].notification_type,
          monetary_due.rows[0].monetary_due_id,
        ]
      );
      res.status(200).json(monetary_due.rows[0]);
    } else {
      const monetary_due = await pool.query(
        "UPDATE monetary_dues SET amount=$1,due_date=$2 where req_id=$3 RETURNING *",
        [amount, dueDateObj, due.rows[0].req_id]
      );
      const notif_status = await pool.query(
        "SELECT * from notification_types where type_name='MonetaryDues'"
      );
      const notification = await pool.query(
        ...
        [
          ...
          username,
          "Lab Assistant",
          "A monetary due has been updated",
          notif_status.rows[0].notification_type,
          monetary_due.rows[0].monetary_due_id,
        ]
      );
...
const viewMonetaryDueStudent = async (req, res) => {
  ...
  try {
    ...
    const monetary_dues = await pool.query(
      `SELECT m.monetary_due_id,m.issue_date,m.due_date,m.amount,ds.status_name,l.location_name,u.username,m.damage_quantity,e.equipment_name from monetary_dues m
        join requests r on m.req_id=r.req_id
        join equipments e on r.equipment_id=e.equipment_id
        join locations l on r.location_id=l.location_id
...
const MonetaryDuesLocation = async (req, res) => {
  ...
  try {
    const dues = await pool.query(
      `SELECT m.monetary_due_id,m.due_date,m.issue_date,ds.status_name,e.equipment_name,u1.username,m.amount,m.clear_date,m.damage_quantity from monetary_dues m
        join requests r on m.req_id=r.req_id
        join equipments e on r.equipment_id=e.equipment_id
        join users_in_locations ul on r.location_id=ul.location_id
        join due_statuses ds on m.due_status=ds.due_status
...
const clearMonetaryDue = async (req, res) => {
  ...
  try {
    const due = await pool.query(
      "SELECT * from monetary_dues where monetary_due_id=$1",
      [dueID]
    );
    const requ = await pool.query("SELECT * from requests where req_id=$1", [
      due.rows[0].req_id,
    ...
    const notification = await pool.query(
      ...
      [
        ...
        username,
        "Lab Assistant",
        "A monetary due has been cleared",
        notif_type.rows[0].notification_type,
        dueID,
      ]
    );
    const clear = await pool.query(
      "UPDATE monetary_dues SET due_status=$1,clear_date=CURRENT_DATE,receiver_id=$2 where monetary_due_id=$3 RETURNING *",
      [duestatus.rows[0].due_status, user_id.rows[0].user_id, dueID]
    );
    res.status(200).json(clear.rows[0]);
  } catch (err) {
...
const createClearanceRequest = async (req, res) => {
  ...
  const clearance_status = await pool.query(
    "SELECT * from clearance_request_status where status_name='Waiting for Superadmin Approval'"
  );
  try {
    const clearance = await pool.query(
      "INSERT INTO clearance_request(user_id,level,term,request_date,clearance_status) values($1,$2,$3,CURRENT_DATE,$4) RETURNING *",
      [
        user_id.rows[0].user_id,
        level,
        term,
        clearance_status.rows[0].clearance_status,
      ]
    );
    const notif_type = await pool.query(
      "SELECT * from notification_types where type_name='Clearance'"
    );
    //console.log(notif_type);
    const notification = await pool.query(
      "INSERT INTO notifications(receiver_id,sender_name,sender_role,notification,notification_time,notification_type,type_id) VALUES ($1,$2,$3,$4,now(),$5,$6) RETURNING *",
      [
        superadmin.rows[0].user_id,
        username,
        "Student",
        "A clearance request has been sent",
        notif_type.rows[0].notification_type,
        clearance.rows[0].clearance_req_id,
      ]
    );
...
const checkClearanceRequests = async (req, res) => {
  try {
    const clearances = await pool.query(
      "SELECT c.clearance_req_id,u.username,c.level,c.term,c.request_date,cr.status_name from clearance_request c join users u on c.user_id=u.user_id join clearance_request_status cr on c.clearance_status=cr.clearance_status"
    );
    res.status(200).json(clearances.rows);
  } catch (err) {
    res.status(400).json({ error: err.message });
