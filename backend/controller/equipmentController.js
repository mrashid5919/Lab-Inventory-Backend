...
const showEquipments = async (req,res) => {
  ...
  const addNewEquipment = async (req,res) => {
    ...
    let NewEquipment=await pool.query("INSERT INTO equipments (equipment_name, type, cost, descript, borrowed, available, demand, permit) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *",[name,category,cost,description,0,quant,1,permit]);
    await pool.query("INSERT INTO equipments_in_locations (equipment_id, location_id, quantity, loan) VALUES ($1, $2, $3, $4)",[NewEquipment.rows[0].equipment_id,1,quant,0]);
    try{
